    ...
    <div className="p-8 space-y-10 flex-1">
     ...
     <div className="bg-slate-800 border border-white/5 rounded-[2rem] p-6 space-y-6">
      ...
      <div className="space-y-4">
       <input type="range" min="-180" max="180" value={config.watermarkRotation} onChange={e => setConfig({ ...config, watermarkRotation: parseInt(e.target.value) })} className="w-full accent-blue-500" />
       <input type="range" min="0" max="1" step="0.05" value={config.watermarkOpacity} onChange={e => setConfig({ ...config, watermarkOpacity: parseFloat(e.target.value) })} className="w-full accent-blue-500" />
      </div>
     </div>
    </div>
   </aside>
