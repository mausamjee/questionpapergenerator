...
import { FileText, Printer, Eye, BookOpen, AlertCircle, CheckCircle2, Settings, Type, Edit, RefreshCw, X, Save, Image as ImageIcon, Upload, Trash2, Plus, Minus, Move, GripHorizontal } from 'lucide-react';
...
const Page = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row overflow-hidden print:overflow-visible bg-slate-100">
      {/* Sidebar Controls */}
      <aside className="no-print w-full md:w-96 bg-white border-r border-slate-200 overflow-y-auto h-screen p-6 shrink-0 z-10 shadow-lg custom-scrollbar">
        ...
        <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <div>
                      <label className="text-[9px] font-bold text-slate-400 block mb-1 flex items-center gap-1">
                        <Move className="w-3 h-3" /> Rotation ({config.watermarkRotation}°)
                      </label>
                      <input
                        type="range"
                        min="-180"
                    ...
                    <div>
                      <label className="text-[9px] font-bold text-slate-400 block mb-1 flex items-center gap-1">
                        <GripHorizontal className="w-3 h-3" /> Opacity ({Math.round(config.watermarkOpacity * 100)}%)
                      </label>
                      <input
                        type="range"
                        min="0"
          ...
          <section>
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto pr-2 custom-scrollbar text-[11px] border border-slate-50 p-2 rounded bg-slate-50">
              {CHAPTERS.map(ch => (
                <label key={ch} className="flex items-center group cursor-pointer py-0.5">
                  <input
                    type="checkbox"
                    checked={config.selectedChapters.includes(ch)}
                    onChange={() => toggleChapter(ch)}
                    className="w-3.5 h-3.5 text-slate-800 rounded border-slate-300 focus:ring-slate-500"
                  />
                  <span className="ml-2 text-slate-600 group-hover:text-slate-900 transition-colors truncate">
                    {ch}
                  </span>
                </label>
              ))}
          ...
          <section>
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Total Marks</h2>
            <div className="flex gap-2">
              {[20, 40, 80].map(m => (
                <button
                  key={m}
                  onClick={() => handleMarksChange(m as any)}
                  className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${config.totalMarks === m ? 'bg-slate-800 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                    }`}
                >
                  {m}m
                </button>
      ...
      {/* Preview Area */}
      <main className="flex-1 overflow-y-auto p-0 md:p-8 print:p-0 print:bg-white bg-slate-200">
        {!paper ? (
          ...
          <div className="paper-container relative">
            <div
              className="a4-paper shadow-2xl print:shadow-none font-serif leading-[1.3] relative overflow-hidden"
              ...
              {(config.watermark || config.watermarkImage) && (
                ...
                >
                  {config.watermarkImage ? (
                    ...
                    <span className="watermark-text">{config.watermark}</span>
                  )}
                </div>
              )}
