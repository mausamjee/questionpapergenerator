...
export default function BulkUploadPage() {
 ...
 const handleBulkUpload = async () => {
  ...
  try {
   ...
   // 2. Map snake_case to camelCase if needed (for Supabase table columns)
   const mappedData = data.map((item: any) => ({
    id: item.id, // Include ID if provided to avoid not-null constraint error
   ...
   // 3. Bulk Insert into Supabase table 'class12'
   ...
   setResult({
    type: 'success',
    message: `Successfully uploaded ${data.length} questions to the 'class12' table!`
   });
    ...
    <div className="mb-10 flex items-center justify-between">
     <div>
      <h1 className="text-3xl font-black text-slate-900 mb-2 flex items-center gap-3">
       ...
       JSON Bulk Upload (Table: class12)
      </h1>
      <p className="text-slate-400 font-bold text-sm uppercase tracking-wider">Add multiple questions at once to class12 table</p>
     </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
       <div className="p-4 bg-slate-900 flex items-center justify-between">
        <div className="flex items-center gap-2">
         ...
         <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">JSON Input Editor</span>
        </div>
        <button
    ...
    ...
       ...
       <div className="p-6 bg-white border-t border-slate-100 flex justify-end">
        ...
        >
         {loading ? (
          ...
          <><Upload className="w-5 h-5" /> Push to class12 table</>
         )}
        </button>
       </div>
      </div>
     ...
     <div className="space-y-6">
      <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-indigo-200">
       ...
       <h3 className="text-xl font-black mb-4 tracking-tight">Bulk Upload (class12) Guide</h3>
       <ul className="space-y-4 text-sm font-bold opacity-90">
        ...
        <li className="flex gap-3">
         ...
         Fields: id (required), subject (defaults to Mathematics), chapter, type, marks, difficulty, content, options, solution, image_url, exam_year.
        </li>
        <li className="flex gap-3">
